...
def default_state() -> dict[str, Any]:
    """Return the default persistent compiler state."""
...
def ensure_knowledge_scaffold_at(knowledge_dir: Path) -> dict[str, Path]:
    """Create a knowledge scaffold rooted at an arbitrary directory."""
    dashboards_dir = knowledge_dir / "dashboards"
    goals_dir = knowledge_dir / "goals"
...
def ensure_knowledge_scaffold() -> None:
    ...
    ensure_knowledge_scaffold_at(KNOWLEDGE_DIR)


def ensure_memory_root_scaffold(root_dir: Path) -> dict[str, Path]:
    """Create the top-level memory root scaffold for a target workspace."""
    ...
    paths = ensure_knowledge_scaffold_at(root_dir / "knowledge")

    state_file = scripts_dir / "state.json"
    if not state_file.exists():
        state_file.write_text(json.dumps(default_state(), indent=2, sort_keys=True), encoding="utf-8")
