...
from .config import SCRIPTS_DIR, now_dt
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.title:
        flush_args.extend(["--title", args.title])
    if args.workspace:
        flush_args.extend(["--workspace", args.workspace])
    if args.repo:
        flush_args.extend(["--repo", args.repo])
    if args.task_ref:
        flush_args.extend(["--task-ref", args.task_ref])
    if args.no_compile_trigger or compile_enabled:
        flush_args.append("--no-compile-trigger")

    from . import compile as compile_module
    from . import flush as flush_module
    from . import lint as lint_module

    flush_result = flush_module.main(flush_args)
    if flush_result:
        raise SystemExit(flush_result)

    if compile_enabled:
        compile_result = compile_module.main([])
        if compile_result:
            raise SystemExit(compile_result)
    if lint_enabled:
        lint_result = lint_module.main(["--autofix"])
        if lint_result:
            raise SystemExit(lint_result)
    return 0
