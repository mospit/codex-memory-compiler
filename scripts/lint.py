...
Runs structural checks by default. Contradiction checks can be enabled through
an external model process in Codex app workflows (documented in AGENTS skills).
...
def check_contradictions_placeholder() -> list[dict]:
    return [
        {
            "severity": "suggestion",
            "check": "contradiction",
            "file": "(system)",
            "detail": (
                "Contradiction check is model-optional in Codex mode. "
                "Use Codex app skill memory-lint for manual semantic review."
            ),
        }
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--structural-only",
        action="store_true",
        help="Skip semantic contradiction reminder check",
    )
    ...
    if not args.structural_only:
        print("  Checking: Contradictions (manual semantic review reminder)...")
        issues = check_contradictions_placeholder()
        all_issues.extend(issues)
        print(f"    Found {len(issues)} issue(s)")
